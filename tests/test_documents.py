"""
Tests for the documents router — endpoint registration and validation.
...
# ---------------------------------------------------------------------------
# Endpoint registration — all document endpoints should return 401, not 404,
...
class TestDocumentsRouterRegistered:
    """Verify all document endpoints are mounted and return 401 (not 404) without auth."""
...
class TestDocumentsAuthErrorShape:
...
class TestDocumentsNotFound:
    """Non-existent routes should still 404."""

    def test_nonexistent_document_route(self):
        r = client.get("/v1/documents/1/nonexistent")

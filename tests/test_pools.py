"""
Tests for the pools router — endpoint registration and validation.
...
# ---------------------------------------------------------------------------
# Endpoint registration — all pool endpoints should return 401, not 404,
...
_AUTH_PATHS = [
    ...
    ("GET", "/v1/pools/1"),
    ("PATCH", "/v1/pools/1"),
    ("POST", "/v1/pools/1/archive"),
]


class TestPoolsRouterRegistered:
    """Verify all pool endpoints are mounted and return 401 (not 404) without auth."""
...
class TestPoolsNotFound:
    """Non-existent routes should still 404."""

    def test_nonexistent_pool_route(self):
        r = client.get("/v1/pools/1/nonexistent")

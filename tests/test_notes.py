"""
Tests for the notes router — endpoint registration and validation.
...
# ---------------------------------------------------------------------------
# Endpoint registration — all note endpoints should return 401, not 404,
...
_AUTH_PATHS = [
    ...
    ("PATCH", "/v1/notes/1"),
    ("DELETE", "/v1/notes/1"),
    ("POST", "/v1/notes/1/close-issue"),
    ("POST", "/v1/notes/1/reopen-issue"),
]


class TestNotesRouterRegistered:
    """Verify all note endpoints are mounted and return 401 (not 404) without auth."""
...
class TestNotesNotFound:
    ...
    def test_nonexistent_note_route(self):
        r = client.get("/v1/notes/1/nonexistent")
